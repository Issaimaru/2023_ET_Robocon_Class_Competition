...
void sound(int freq,int duration,int volume){//îÕàÕ 31-2100[Hz]ÅC256(2.56[sec])ÅC0-100
    ecrobot_sound_tone(freq,duration,volume); //âπÇèoÇ∑ÇÊ
}

void seesaw(){
    int distance=ecrobot_get_sonar_sensor(PORT_SONAR);//í¥âπîgÉZÉìÉTÇ©ÇÁãóó£ÇéÊìæ
    
    while(distance>30){//ÉQÅ[ÉgÇÃíºëOÇ‹Ç≈êiÇﬁÇÊ
        systick_wait_ms(100U);
    	display_goto_xy(0,2); display_int(distance, 6); display_update();
        distance=ecrobot_get_sonar_sensor(PORT_SONAR);//í¥âπîgÉZÉìÉTÇ©ÇÁãóó£ÇéÊìæ
    }
    sound(500,100,100);//âπÇèoÇµÇƒímÇÁÇπÇÈÇÊ
    int gate_distance=distance;//ÉQÅ[ÉgÇÃà íuÇîcà¨
    
    int seesaw_distance=300;//ÉQÅ[ÉgÇ©ÇÁÉVÅ[É\Å[Ç‹Ç≈ÇÃãóó£[cm]
    cmd_forward = 60; //ë¨ìxÇÃïœçX
    while(-(odd_distance-gate_distance) < seesaw_distance){//ÉVÅ[É\Å[ÇÃíºëOÇ‹Ç≈êiÇﬁ
    	display_goto_xy(0,2); display_int(-((odd_distance-gate_distance)), 6); display_update();
        systick_wait_ms(100U);
    }
    sound(800,100,100);//âπÇèoÇµÇƒímÇÁÇπÇÈÇÊ
    seesaw_distance=odd_distance;//ÉVÅ[É\Å[ÇÃà íuÇîcà¨
    
    while((odd_distance-seesaw_distance)<100){//ÉVÅ[É\Å[ÇÃíÜêSÇ‹Ç≈êiÇﬁ
        systick_wait_ms(100U);  
    }
    sound(1200,100,100);//âπÇèoÇµÇƒímÇÁÇπÇÈÇÊ
    
    cmd_forward=0;
    systick_wait_ms(3000U);//ãÍÇµÇ›ÇÃìæì_ë_Ç¢
    cmd_forward=30;
    sound(1600,100,100);//âπÇèoÇµÇƒímÇÁÇπÇÈÇÊ
    
    int centor_distance=odd_distance;//íÜêSÇÃà íuÇîcà¨
    while((odd_distance-centor_distance)<200){//ÉVÅ[É\Å[Çâ∫ÇËÇÈÇ‹Ç≈êiÇﬁ
        systick_wait_ms(100U); 
    }
    sound(2000,100,100);//âπÇèoÇµÇƒímÇÁÇπÇÈÇÊ

    cmd_forward=60;
    int edge_distance=odd_distance;//ÉVÅ[É\Å[ÇÃí[ÇÃà íuÇîcà¨
